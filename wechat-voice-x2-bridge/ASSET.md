...
当前版本为 `0.4.0`。核心监听采用“低层钩子 + `GetAsyncKeyState` 轮询后备”双通道；守护程序通过 PID、进程启动时间和 15 秒心跳共同判断健康。计划任务直接运行隐藏 PowerShell，不再依赖可能挂起的 VBS 中间层。默认依靠计划任务自动启动和恢复；桌面 `鼠标语音输入-启动修复.lnk` 仅用于异常时手动重启并检查健康状态。
